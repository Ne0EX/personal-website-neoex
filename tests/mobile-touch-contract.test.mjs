...
// ─────────────────────────────────────────────────────────────────────────────
// (f) WorldlineGlobe.tsx: RAF loop (not setInterval) in render section
// ─────────────────────────────────────────────────────────────────────────────
test('WorldlineGlobe.tsx: render loop uses requestAnimationFrame, not setInterval', () => {
  ...
  assert.match(
    ...
    'WorldlineGlobe.tsx must use requestAnimationFrame for the render loop',
  )
  assert.match(
    src,
    /cancelAnimationFrame\(/,
    'WorldlineGlobe.tsx must use cancelAnimationFrame for cleanup',
  )

  // setInterval must NOT appear in the render loop context.
  // A comment reference is allowed (the spec says "was setInterval") but an
  // active call must not exist. We strip comments first.
  const stripped = src.replace(/\/\/[^\n]*/g, '').replace(/\/\*[\s\S]*?\*\//g, '')
  assert.doesNotMatch(
    stripped,
    /setInterval\s*\(/,
    'WorldlineGlobe.tsx must not use setInterval() in active code (RAF replaced it)',
  )
})
