...
export async function readPhotoMedia(keySegments: unknown): Promise<Response> {
  ...
  try {
    ...
    if (entryError || !entry || typeof entry.id !== 'string') return imageNotFound()
    ...
    if (assetError || variants?.[variant.size]?.[variant.format] !== variant.key) {
    ...
    if (downloadError || !image) return imageNotFound()
