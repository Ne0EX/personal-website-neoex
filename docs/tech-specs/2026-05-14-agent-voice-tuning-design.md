...
4. **Hook regex edge cases** — current pattern matches codenames adjacent to whitespace / comma / em-dash. Names in prose ("Polaris's territory") may produce false positives via the trailing apostrophe / `s` not being in the boundary class. Acceptable for pilot; Canopus to harden during implementation.
