...
held-pending (Peat) — security decision · **P1 draft-image CDN reachability** (whole-site QA needs-peat): draft photos' image variants live in the PUBLIC `photos` bucket, so the exact (hash-bearing, unlistable) URL is reachable by anyone who has it — the spec §4.3 "capability-URL exposure, accepted" call. Peat to confirm: (a) accept (low risk — unguessable hash, personal site, no list/REST leak) or (b) move draft variants to a private bucket + signed URLs (more work). blocked_on: Peat · block_until: 2026-06-26. NOT blocking; surfaced before any production deploy.
