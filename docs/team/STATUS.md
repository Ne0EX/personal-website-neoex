...
flagged (2026-06-14) · **scripts/recover-dscf0344.ts** was removed post-run (git clean) — recovery is one-shot, no artifact. · gallery shows "AWAITING IMAGE" for the migrated no-original photos (DSCF0002-0005) + draft DSCF0344; real uploads via QuickUploadBar show immediately (CHATGPTIMAGE renders). · lesson `feedback_test_cleanup_blast_radius` now embedded in workflow cleanup prompts (capture exact baseline + sacred-slug list, delete only own IDs, assert exact return).
